...
const CATEGORY_COLORS: Record<TemplateCategory, string> = {
  ...
  [TemplateCategory.TRANSACTIONAL]: "bg-gray-500/20 text-gray-400",
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">{Array.isArray(templates) ? templates.length : 0} templates</p>
            </div>
          </div>
          <button
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 bg-white/5 rounded-lg p-1">
            <button
              ...
              className={cn("p-2 rounded-md transition-colors", viewMode === "grid" ? "bg-blue-500/20 text-blue-400" : "text-gray-400 hover:text-white")}
            >
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setViewMode("list")}
              className={cn("p-2 rounded-md transition-colors", viewMode === "list" ? "bg-blue-500/20 text-blue-400" : "text-gray-400 hover:text-white")}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {templates.map((template) => (
              <motion.div
                ...
                className="group bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-blue-500/50 transition-all cursor-pointer"
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm text-gray-400 truncate mb-3">{template.subject}</p>
                  ...
                  {template.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-3">
                      ...
                      {template.tags.length > 3 && (
                        <span className="px-2 py-0.5 text-gray-500 text-xs">+{template.tags.length - 3}</span>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-2">
            {templates.map((template) => (
              <motion.div
                ...
                className="flex items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-lg hover:border-blue-500/50 transition-all cursor-pointer"
                onClick={() => onSelectTemplate?.(template)}
              >
                <div className="p-2 rounded-lg bg-gradient-to-br from-blue-500/20 to-purple-500/20">
                  <Mail className="w-5 h-5 text-blue-400" />
