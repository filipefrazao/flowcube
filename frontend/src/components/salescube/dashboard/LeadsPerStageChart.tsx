  ...
  if (data.length === 0) {
    return (
      <div className="bg-surface/60 border border-border/50 rounded-2xl p-6">
        <h3 className="text-sm font-semibold text-text-primary mb-1">Leads por Etapa</h3>
        <p className="text-xs text-text-muted mb-4">Leads por etapa do pipeline</p>
  ...
  return (
    <div className="bg-surface/60 border border-border/50 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-text-primary">Leads por Etapa</h3>
          <p className="text-xs text-text-muted">Leads por etapa do pipeline</p>
        </div>
          ...
          >
            ...
            <Bar dataKey="count" radius={[0, 6, 6, 0]} barSize={24}>
              {data.map((entry, index) => (
                <Cell key={index} fill={entry.color || "#F59E0B"} fillOpacity={0.8} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
