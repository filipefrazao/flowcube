...
function ColumnSkeleton() {
  return (
    <div className="flex flex-col w-[290px] min-w-[290px] bg-gray-800/50 rounded-xl border border-gray-700/50 animate-pulse">
      <div className="px-3 py-3 border-b border-gray-700/50">
        <div className="flex items-center gap-2">
          <div className="w-full h-3 bg-gray-700 rounded" />
          <div className="w-8 h-3 bg-gray-700 rounded-full" />
        </div>
      </div>
...
function QuickAddLead({
  ...
  return (
    <div className="bg-gray-900 border border-indigo-500/30 rounded-lg p-2.5 space-y-2">
      <input
        ref={inputRef}
        type="text"
        ...
        className="w-full bg-gray-800 border border-gray-700 rounded px-2.5 py-1.5 text-xs text-gray-100 placeholder-gray-500 focus:border-indigo-500 outline-none"
      />
      <input
        type="tel"
      ...
      <div className="flex justify-end gap-1.5">
        <button onClick={onCancel} className="px-2 py-1 text-[10px] text-gray-400 hover:text-gray-200 transition-colors">
          Cancelar
        </button>
        <button
          onClick={handleSubmit}
          disabled={!name.trim() || saving}
          className="px-2.5 py-1 text-[10px] bg-indigo-600 hover:bg-indigo-700 text-white rounded disabled:opacity-50 transition-colors"
        >
          {saving ? "..." : "Criar"}
        </button>
      </div>
...
function LeadCard({
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        "group bg-gray-900 border border-gray-700/50 rounded-lg p-3 cursor-pointer select-none",
        "hover:border-indigo-500/50 hover:shadow-lg hover:shadow-indigo-500/5 transition-all duration-150",
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          ...
          <span className="text-sm font-medium text-gray-100 truncate">{lead.name}</span>
        </div>
      ...
      <div className="flex items-center justify-between mt-2 ml-[18px]">
        <div className="flex items-center gap-2 min-w-0">
          {lead.assigned_to_name ? (
            ...
            <span className="text-[10px] text-gray-700">Sem resp.</span>
          )}
          {lead.source && lead.source !== "manual" && (
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-500 border border-gray-700/50 truncate max-w-[70px]">
              {sourceLabel}
            </span>
          )}
        </div>
        {formatBRL(lead.value) && (
          <span className="text-xs font-semibold text-emerald-400 shrink-0">{formatBRL(lead.value)}</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2.5 border-b border-gray-700/50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[10px] bg-gray-700/80 text-gray-300 px-1.5 py-0.5 rounded-full font-medium shrink-0">
              {col.count}
            </span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            {hasValue && (
              <span className="text-[10px] text-indigo-400 font-semibold">{hasValue}</span>
            )}
            <button
              onClick={() => setShowAddForm(true)}
              className="p-1 text-gray-500 hover:text-indigo-400 hover:bg-gray-700/50 rounded transition-all"
        ...
        {col.probability > 0 && col.probability < 100 && (
          <div className="mt-1.5 flex items-center gap-2">
            ...
            <span className="text-[9px] text-gray-500 shrink-0">{col.probability}%</span>
          </div>
        )}
      </div>
      ...
      >
        ...
        {col.leads.length === 0 && !showAddForm && (
          <div className="text-center py-6">
            <LayoutGrid className="w-6 h-6 text-gray-700 mx-auto mb-1.5" />
            <p className="text-xs text-gray-600">Nenhum lead</p>
            <button
              onClick={() => setShowAddForm(true)}
              className="mt-1.5 text-[10px] text-indigo-400 hover:text-indigo-300 transition-colors"
            >
              + Adicionar
            </button>
          </div>
      ...
      {col.leads.length > 0 && col.leads.length < col.count && !isLoadingMore && (
        <div className="px-3 py-1.5 border-t border-gray-700/30 text-center">
...
export default function KanbanPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <div className="flex-shrink-0 px-6 py-3 border-b border-gray-700 bg-gray-900/50">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="relative w-64">
            ...
            <input
              ...
              className="w-full bg-gray-800 border border-gray-700 rounded-lg pl-10 pr-3 py-1.5 text-sm text-gray-100 placeholder-gray-500 focus:border-indigo-500 outline-none transition-colors"
            />
          </div>
        </div>
          ...
          <div className="flex items-center gap-1">
            {pipelines.length > 4 && (
              <button
                ...
                className="p-1 text-gray-500 hover:text-gray-300 shrink-0 transition-colors"
              >
                ...
                return (
                  ...
                  >
                    ...
                    {isActive && p.is_default && (
                      <span className="ml-1.5 text-[9px] text-indigo-200 opacity-70">padrao</span>
                    )}
                  </button>
                );
              })}
      ...
      {loading ? (
        ...
        <div className="flex items-center justify-center flex-1 text-gray-500">
          <div className="text-center">
            <LayoutGrid className="w-12 h-12 text-gray-700 mx-auto mb-3" />
