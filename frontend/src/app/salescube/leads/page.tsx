...
function LeadDrawer({ leadId, stages, pipelines, onClose, onUpdated }: LeadDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-2xl bg-background-secondary border-l border-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-border flex-shrink-0">
          {loading ? (
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="w-40 h-5 bg-surface-hover rounded animate-pulse mb-1" />
                <div className="w-24 h-3 bg-surface-hover rounded animate-pulse" />
              </div>
            </div>
          ) : lead ? (
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-text-primary font-semibold text-sm flex-shrink-0">
                {lead.name.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0">
                <h2 className="text-lg font-semibold text-text-primary truncate">{lead.name}</h2>
          ...
          <button onClick={onClose} className="p-2 text-text-secondary hover:text-text-primary transition-colors flex-shrink-0">
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Tabs */}
        <div className="flex border-b border-border px-4 flex-shrink-0 overflow-x-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <p className="text-center text-text-muted py-10">Lead nao encontrado</p>
          ) : (
            <>
              {/* ===== TAB: DADOS ===== */}
              {activeTab === "dados" && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                        className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-text-primary focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors" />
                    </div>
                    <div>
                    ...
                    <div>
                      <label className="text-xs text-text-muted mb-1 block">CPF</label>
                      <input type="text" value={formData.cpf} onChange={(e) => setFormData({ ...formData, cpf: e.target.value })}
                        className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-text-primary focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors" />
                    </div>
                    <div>
                      <label className="text-xs text-text-muted mb-1 block">Fonte</label>
                      <select value={formData.source} onChange={(e) => setFormData({ ...formData, source: e.target.value })}
                        className="w-full bg-surface border border-border text-text-primary rounded-lg px-3 py-2 text-sm focus:border-primary outline-none transition-colors">
                    ...
                    <div>
                      <label className="text-xs text-text-muted mb-1 block">Estagio</label>
                    ...
                    <div>
                      <label className="text-xs text-text-muted mb-1 block">Responsavel</label>
                      <input type="text" value={formData.assigned_to || ""} onChange={(e) => setFormData({ ...formData, assigned_to: e.target.value || null })}
                        ...
                        className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors" />
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={3}
                      className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-text-primary resize-none focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {lead.responsibles_names && lead.responsibles_names.length > 0 && (
                      <div>
                        <label className="text-xs text-text-muted mb-1 block">Responsaveis</label>
                    ...
                    {lead.squad_names && lead.squad_names.length > 0 && (
                      <div>
                        <label className="text-xs text-text-muted mb-1 block">Squads</label>
                        <div className="flex flex-wrap gap-1">
                    ...
                    {lead.franchise_names && lead.franchise_names.length > 0 && (
                      <div>
                        <label className="text-xs text-text-muted mb-1 block">Franquias</label>
                        <div className="flex flex-wrap gap-1">
                ...
                <div className="space-y-4">
                  <div className="bg-surface border border-border rounded-lg p-3">
                      ...
                      {NOTE_TYPE_OPTIONS.map((opt) => (
                        <button key={opt.value} onClick={() => setNewNote({ ...newNote, note_type: opt.value })}
                          className={cn(
                            ...
                            newNote.note_type === opt.value ? "bg-primary text-gray-900" : "text-text-secondary hover:text-text-primary"
                          )}>
                    ...
                    <textarea value={newNote.content} onChange={(e) => setNewNote({ ...newNote, content: e.target.value })}
                      ...
                      className="w-full bg-background-secondary border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted resize-none mb-2 focus:border-primary outline-none" />
                  ...
                  {(lead.activities || []).map((activity, i) => (
                    <div key={activity.id} className="flex gap-3 py-2">
                      <div className="flex flex-col items-center">
                        ...
                        {i < (lead.activities?.length || 0) - 1 && <div className="w-px flex-1 bg-surface mt-1" />}
                      </div>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-sm text-text-primary">
                            ...
                            {activity.action === "comment_added" && (
                              <>Comentario: <span className="text-text-secondary italic">{activity.new_value}</span></>
                            )}
                  ...
                  {!showTaskForm ? (
                    ...
                    <div className="bg-surface border border-border rounded-lg p-3 space-y-2">
                      <input type="text" placeholder="Titulo *" value={newTask.title} onChange={(e) => setNewTask({ ...newTask, title: e.target.value })}
                        className="w-full bg-background-secondary border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted" autoFocus />
                      ...
                      <div className="flex gap-2">
                        <input type="date" value={newTask.due_date} onChange={(e) => setNewTask({ ...newTask, due_date: e.target.value })}
                          className="flex-1 bg-background-secondary border border-border rounded-lg px-3 py-2 text-sm text-text-primary" />
                        <select value={newTask.priority} onChange={(e) => setNewTask({ ...newTask, priority: e.target.value })}
                          className="bg-background-secondary border border-border text-text-primary rounded-lg px-3 py-2 text-sm">
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        <button onClick={handleAddTask} disabled={!newTask.title.trim()}
                          className="px-3 py-1.5 bg-primary hover:bg-primary-hover text-gray-900 rounded-lg text-xs disabled:opacity-50">Criar</button>
                      </div>
                    </div>
                  )}
                  {(lead.tasks || []).map((task) => {
                ...
                <div className="space-y-3">
                  {(lead.sales || []).map((sale) => (
                    <div key={sale.id} className="bg-surface border border-border rounded-lg p-3">
                      ...
                      <div className="text-xs text-text-muted mt-1">{formatDate(sale.created_at)}</div>
                    </div>
                  ))}
        ...
        <div className="flex items-center justify-between p-4 border-t border-border flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSave} disabled={saving || !formData.name}
              className="flex items-center gap-1 px-4 py-2 bg-primary hover:bg-primary-hover text-gray-900 rounded-lg text-sm transition-colors disabled:opacity-50">
    ...
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-text-secondary">
            ...
            {selectedIds.size > 0 && (
              <span className="text-primary ml-2">({selectedIds.size} selecionados)</span>
            )}
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="relative">
            <button onClick={() => setShowPresetMenu(!showPresetMenu)}
              className="flex items-center gap-1 px-3 py-2 text-text-secondary hover:text-text-primary text-sm transition-colors border border-border rounded-lg hover:bg-surface-hover">
              <Bookmark className="w-4 h-4" /> Filtros salvos
            </button>
            {showPresetMenu && (
              <>
                <div className="fixed inset-0 z-30" onClick={() => setShowPresetMenu(false)} />
                <div className="absolute right-0 top-full mt-1 w-72 bg-surface border border-border rounded-lg shadow-xl z-40 p-3">
                  <p className="text-xs text-text-muted font-medium mb-2">FILTROS SALVOS</p>
                  {presets.length === 0 && (
                    <p className="text-xs text-text-muted py-2">Nenhum filtro salvo</p>
                  )}
                  {presets.map((preset) => (
                    <div key={preset.id} className="flex items-center justify-between py-1.5 group">
                      <button onClick={() => handleLoadPreset(preset)} className="text-sm text-text-primary hover:text-primary truncate flex-1 text-left">
                        {preset.name}
                      </button>
                      <button onClick={() => handleDeletePreset(preset.id)} className="text-text-muted hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
                  <div className="border-t border-border mt-2 pt-2">
                    <div className="flex gap-2">
                      <input type="text" value={newPresetName} onChange={(e) => setNewPresetName(e.target.value)}
                        ...
                        className="flex-1 bg-background-secondary border border-border rounded px-2 py-1 text-xs text-text-primary placeholder-text-muted" />
                      <button onClick={handleSavePreset} disabled={!newPresetName.trim()}
                        className="px-2 py-1 bg-primary text-gray-900 rounded text-xs disabled:opacity-50 hover:bg-primary-hover">
          ...
          <button onClick={handleExportCSV} className="flex items-center gap-1 px-3 py-2 text-text-secondary hover:text-text-primary text-sm transition-colors border border-border rounded-lg hover:bg-surface-hover">
            <Download className="w-4 h-4" /> CSV
          </button>
          <button className="flex items-center gap-1 px-3 py-2 text-text-secondary hover:text-text-primary text-sm transition-colors border border-border rounded-lg hover:bg-surface-hover cursor-not-allowed opacity-50" title="Em breve">
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-3 bg-primary/10 border border-primary/20 rounded-lg px-4 py-2.5 flex-wrap">
          <span className="text-sm text-primary font-medium">{selectedIds.size} selecionados</span>
          <div className="h-4 w-px bg-primary/30" />
          <button onClick={() => setShowBulkMoveModal(true)}
            className="flex items-center gap-1 px-3 py-1.5 text-xs bg-primary text-gray-900 rounded-lg hover:bg-primary-hover transition-colors">
            <ArrowRight className="w-3 h-3" /> Mover Estagio
          </button>
          <button onClick={() => setShowBulkAssignModal(true)}
            className="flex items-center gap-1 px-3 py-1.5 text-xs bg-surface-hover text-text-primary rounded-lg hover:bg-surface-hover transition-colors">
          ...
          <button onClick={() => setSelectedIds(new Set())} className="ml-auto text-xs text-text-secondary hover:text-text-primary">
            Limpar selecao
          </button>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[240px] max-w-lg">
          ...
          <input type="text" placeholder="Buscar por nome, email, telefone, empresa..."
            ...
            className="w-full bg-surface border border-border rounded-lg pl-10 pr-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors" />
          {filters.search && (
            <button onClick={() => updateFilter("search", "")} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-primary">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
        ...
        <button onClick={() => setShowFilters(!showFilters)}
          ...
          Filtros
          {activeFilterCount > 0 && (
            <span className="w-5 h-5 rounded-full bg-primary text-gray-900 text-[11px] flex items-center justify-center font-medium">
              {activeFilterCount}
            </span>
          )}
      ...
      {showFilters && (
        <div className="bg-surface/50 border border-border rounded-xl p-4 space-y-4 animate-in slide-in-from-top-2 duration-200">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {/* Nome */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Nome do Lead</label>
              <input type="text" value={filters.search} onChange={(e) => updateFilter("search", e.target.value)}
                placeholder="Buscar nome..." className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" />
            </div>
            {/* Email */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Email</label>
              <input type="text" value={filters.email} onChange={(e) => updateFilter("email", e.target.value)}
                placeholder="Buscar email..." className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" />
            </div>
            {/* Telefone */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Telefone</label>
              <input type="text" value={filters.phone} onChange={(e) => updateFilter("phone", e.target.value)}
                placeholder="Buscar telefone..." className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" />
            </div>
            {/* Pipeline */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Pipeline / Quadro</label>
              <select value={filters.pipeline} onChange={(e) => { updateFilter("pipeline", e.target.value); updateFilter("stage", ""); }}
                className="w-full bg-background-secondary border border-border text-text-primary rounded-lg px-3 py-1.5 text-sm focus:border-primary outline-none">
            ...
            {/* Estagio */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Estagio / Coluna</label>
            ...
            {/* Sem Coluna */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Sem Coluna</label>
              <label className="flex items-center gap-2 bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm cursor-pointer">
                ...
                <span className="text-text-primary">Apenas sem coluna</span>
              </label>
            </div>
            {/* Fonte */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Origem / Source</label>
            ...
            {/* Origem */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Origem</label>
            ...
            {/* Squad */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Squad</label>
            ...
            {/* Franquia */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Franquia</label>
            ...
            {/* Responsavel */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Responsavel</label>
              <input type="text" value={filters.assignedTo} onChange={(e) => updateFilter("assignedTo", e.target.value)}
                placeholder="ID do usuario..." className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" />
            </div>
            {/* Score Min */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Score Minimo</label>
              <input type="number" min={0} max={100} value={filters.scoreMin} onChange={(e) => updateFilter("scoreMin", e.target.value)}
                placeholder="0" className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" />
            </div>
            {/* Score Max */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Score Maximo</label>
              <input type="number" min={0} max={100} value={filters.scoreMax} onChange={(e) => updateFilter("scoreMax", e.target.value)}
                placeholder="100" className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" />
            </div>
            {/* Criado de */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Criado de</label>
              <input type="date" value={filters.createdFrom} onChange={(e) => updateFilter("createdFrom", e.target.value)}
                className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary focus:border-primary outline-none" />
            </div>
            {/* Criado ate */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Criado ate</label>
              <input type="date" value={filters.createdTo} onChange={(e) => updateFilter("createdTo", e.target.value)}
                className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary focus:border-primary outline-none" />
            </div>
            {/* Atualizado de */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Ult. Alteracao de</label>
              <input type="date" value={filters.updatedFrom} onChange={(e) => updateFilter("updatedFrom", e.target.value)}
                className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary focus:border-primary outline-none" />
            </div>
            {/* Atualizado ate */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Ult. Alteracao ate</label>
              <input type="date" value={filters.updatedTo} onChange={(e) => updateFilter("updatedTo", e.target.value)}
                className="w-full bg-background-secondary border border-border rounded-lg px-3 py-1.5 text-sm text-text-primary focus:border-primary outline-none" />
            </div>
            {/* Ordenacao */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Ordenacao</label>
            ...
            {/* Qtd por pagina */}
            <div>
              <label className="text-[11px] text-text-muted font-medium mb-1 block uppercase tracking-wide">Qtd por pagina</label>
              ...
              <tr className="border-b border-border text-left">
                <th className="px-3 py-3 w-10">
                  <input type="checkbox" checked={selectedIds.size === leads.length && leads.length > 0} onChange={toggleSelectAll}
                    className="rounded border-border bg-background-secondary text-primary focus:ring-primary" />
                </th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider">Nome</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider">Email</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider hidden lg:table-cell">Telefone</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider">Estagio</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider hidden xl:table-cell">Pipeline</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider hidden lg:table-cell">Fonte</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider hidden xl:table-cell">Origem</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider">Score</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider">Valor</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider hidden xl:table-cell">Responsavel</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider hidden 2xl:table-cell">Tags</th>
                <th className="px-3 py-3 text-text-secondary font-medium text-xs uppercase tracking-wider">Criacao</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={13} className="px-4 py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-text-muted font-medium">Nenhum lead encontrado</p>
                      <p className="text-text-muted text-xs">Tente ajustar os filtros ou criar um novo lead</p>
                    </div>
                  </td>
                </tr>
              ) : (
                leads.map((lead) => (
                  <tr
                    key={lead.id}
                    className={cn(
                      "border-b border-border/50 hover:bg-surface-hover/30 transition-colors cursor-pointer",
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <span className="text-text-primary font-medium truncate block max-w-[180px]">{lead.name}</span>
                    </td>
                    <td className="px-3 py-2.5">
                      <span className="text-text-secondary truncate block max-w-[180px]">{lead.email || "-"}</span>
                    </td>
                    ...
                    <td className="px-3 py-2.5 hidden xl:table-cell">
                      <span className="text-text-muted text-xs">{getPipelineName(lead)}</span>
                    </td>
                    <td className="px-3 py-2.5 hidden lg:table-cell">
                      <span className="text-text-secondary text-xs">{formatSourceLabel(lead.source)}</span>
                    </td>
                    <td className="px-3 py-2.5 hidden xl:table-cell">
                      <span className="text-text-secondary text-xs">{lead.origin_name || "-"}</span>
                    </td>
                    <td className="px-3 py-2.5">
                      <span className={cn(
                    ...
                    <td className="px-3 py-2.5">
                      <span className="text-primary font-medium text-xs">{formatCurrency(lead.value)}</span>
                    </td>
                    <td className="px-3 py-2.5 hidden xl:table-cell">
                      <span className="text-text-secondary text-xs truncate block max-w-[100px]">{lead.assigned_to_name || "-"}</span>
                    </td>
                    <td className="px-3 py-2.5 hidden 2xl:table-cell">
                      <div className="flex gap-1 flex-wrap max-w-[120px]">
                        {((lead as any).tags || []).slice(0, 2).map((tag: any, i: number) => (
                          <span key={i} className="text-[10px] px-1.5 py-0.5 rounded-full bg-surface-hover text-text-secondary">
                            {typeof tag === "string" ? tag : tag.name || "tag"}
                          </span>
                        ))}
                        {((lead as any).tags || []).length > 2 && (
                          <span className="text-[10px] text-text-muted">+{(lead as any).tags.length - 2}</span>
                        )}
                        {(!(lead as any).tags || (lead as any).tags.length === 0) && (
                    ...
                    <td className="px-3 py-2.5">
                      <span className="text-text-muted text-xs whitespace-nowrap">{formatDate(lead.created_at)}</span>
                    </td>
                  </tr>
                ))
              )}
                ...
                p === "..." ? (
                  ...
                  <button key={p} onClick={() => setPage(p as number)}
                    className={cn(
                      ...
                      page === p ? "bg-primary text-gray-900" : "text-text-secondary hover:text-text-primary hover:bg-surface-hover"
                    )}>
                    {p}
                  </button>
                )
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowAddModal(false)}>
          <div className="bg-surface border border-border rounded-xl p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-text-primary">Novo Lead</h2>
              <button onClick={() => setShowAddModal(false)} className="text-text-secondary hover:text-text-primary"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-3">
              <input type="text" placeholder="Nome *" value={newLead.name} onChange={(e) => setNewLead({ ...newLead, name: e.target.value })}
                className="w-full bg-background-secondary border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" autoFocus />
              <div className="grid grid-cols-2 gap-3">
                <input type="email" placeholder="Email" value={newLead.email} onChange={(e) => setNewLead({ ...newLead, email: e.target.value })}
                  className="bg-background-secondary border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" />
              ...
              <input type="text" placeholder="Empresa" value={newLead.company} onChange={(e) => setNewLead({ ...newLead, company: e.target.value })}
                className="w-full bg-background-secondary border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-primary outline-none" />
              <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowAddModal(false)} className="px-4 py-2 text-sm text-text-secondary hover:text-text-primary">Cancelar</button>
              <button onClick={handleAddLead} disabled={!newLead.name}
                className="px-4 py-2 bg-primary hover:bg-primary-hover text-gray-900 rounded-lg text-sm disabled:opacity-50 transition-colors">Salvar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showBulkMoveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowBulkMoveModal(false)}>
          <div className="bg-surface border border-border rounded-xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-text-primary mb-4">Mover {selectedIds.size} leads</h2>
            <select value={bulkMoveStage} onChange={(e) => setBulkMoveStage(e.target.value)}
              className="w-full bg-background-secondary border border-border text-text-primary rounded-lg px-3 py-2 text-sm mb-4">
              <option value="">Selecione o estagio</option>
              {pipelines.map((pipeline) => (
                <optgroup key={pipeline.id} label={pipeline.name}>
                  {allStages.filter((s) => s.pipeline === pipeline.id).map((s) => (
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowBulkMoveModal(false)} className="px-4 py-2 text-sm text-text-secondary">Cancelar</button>
              <button onClick={handleBulkMove} disabled={!bulkMoveStage}
                className="px-4 py-2 bg-primary text-gray-900 rounded-lg text-sm disabled:opacity-50 hover:bg-primary-hover transition-colors">Mover</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showBulkAssignModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowBulkAssignModal(false)}>
          <div className="bg-surface border border-border rounded-xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-text-primary mb-4">Atribuir Responsavel</h2>
            <p className="text-sm text-text-secondary mb-3">{selectedIds.size} leads selecionados</p>
            <input type="text" placeholder="ID do usuario (vazio = remover)" value={bulkAssignUser} onChange={(e) => setBulkAssignUser(e.target.value)}
              className="w-full bg-background-secondary border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted mb-4 focus:border-primary outline-none" />
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowBulkAssignModal(false)} className="px-4 py-2 text-sm text-text-secondary">Cancelar</button>
              <button onClick={handleBulkAssign}
                className="px-4 py-2 bg-primary text-gray-900 rounded-lg text-sm hover:bg-primary-hover transition-colors">Atribuir</button>
            </div>
          </div>
        </div>
      )}
