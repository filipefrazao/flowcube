...
const ENROLLMENT_STATUS_OPTIONS = [
  ...
  { value: "sem_contato", label: "Sem Contato", color: "bg-gray-500/20 text-text-secondary border-gray-500/30" },
...
const CLASS_STATUS = {
  ...
  finalizada: { label: "Finalizada", color: "bg-gray-500/20 text-text-secondary border-gray-500/30" },
...
export default function TurmaDetailPage() {
  ...
  if (!classData) {
    return (
      <div className="flex h-screen bg-background">
        <AppSidebar />
        <div className="flex-1 flex items-center justify-center text-text-secondary">Turma nao encontrada.</div>
      </div>
    );
  }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="border-b border-border bg-background px-6 py-4 shrink-0">
          <div className="flex items-center gap-4 mb-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3">
                <h1 className="text-xl font-semibold text-text-primary">{classData.name}</h1>
            ...
            <button onClick={() => router.push(`/minicube/turmas/${classId}/presenca`)}
              className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-text-primary rounded-lg text-sm font-medium transition-colors">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-background-secondary rounded-lg border border-border p-3">
              <div className="flex items-center gap-2 text-text-secondary text-xs mb-1"><Users className="w-3.5 h-3.5" /> Alunos</div>
              <p className="text-lg font-semibold text-text-primary">{enrollments.length} / {classData.capacity}</p>
            </div>
            <div className="bg-background-secondary rounded-lg border border-border p-3">
              <div className="flex items-center gap-2 text-text-secondary text-xs mb-1"><Calendar className="w-3.5 h-3.5" /> Inicio</div>
              <p className="text-lg font-semibold text-text-primary">{classData.start_date ? new Date(classData.start_date).toLocaleDateString("pt-BR") : "-"}</p>
            </div>
            <div className="bg-background-secondary rounded-lg border border-border p-3">
              <div className="flex items-center gap-2 text-text-secondary text-xs mb-1"><Calendar className="w-3.5 h-3.5" /> Termino</div>
              <p className="text-lg font-semibold text-text-primary">{classData.end_date ? new Date(classData.end_date).toLocaleDateString("pt-BR") : "-"}</p>
            </div>
            <div className="bg-background-secondary rounded-lg border border-border p-3">
              <div className="flex items-center gap-2 text-text-secondary text-xs mb-1"><GraduationCap className="w-3.5 h-3.5" /> Capacidade</div>
              <p className="text-lg font-semibold text-text-primary">{classData.capacity}</p>
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <h2 className="text-base font-semibold text-text-primary">Alunos Matriculados</h2>
              <span className="text-xs text-text-muted bg-surface px-2 py-0.5 rounded-full">{enrollments.length}</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted" />
                <input type="text" placeholder="Buscar aluno..." value={search} onChange={(e) => setSearch(e.target.value)}
                  className="pl-10 pr-4 py-2 bg-background-secondary border border-border rounded-lg text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary transition-colors" />
              </div>
              <button onClick={() => setShowAddStudent(true)}
                className="flex items-center gap-2 px-3 py-2 bg-primary hover:bg-primary-hover text-gray-900 rounded-lg text-sm font-medium transition-colors">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-medium text-text-muted uppercase tracking-wider">Matricula</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-text-muted uppercase tracking-wider">Notas</th>
                ...
                <tbody className="divide-y divide-border/50">
                  {filteredEnrollments.map((e) => (
                    <tr key={e.id} className="hover:bg-surface-hover/40 transition-colors">
                      <td className="px-4 py-3 text-sm font-medium text-text-primary">{e.student_name || "-"}</td>
                      <td className="px-4 py-3">
                        {editingEnrollment === e.id ? (
                          <div className="flex items-center gap-2">
                            <select value={editStatus} onChange={(ev) => setEditStatus(ev.target.value)}
                              className="px-2 py-1 bg-background border border-border rounded text-xs text-text-primary focus:outline-none focus:border-primary">
                      ...
                      <td className="px-4 py-3 text-sm text-text-secondary max-w-[200px] truncate">{e.notes || "-"}</td>
        ...
        {showAddStudent && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-background-secondary rounded-xl border border-border p-6 w-full max-w-lg shadow-2xl max-h-[80vh] flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-text-primary">Adicionar Aluno</h2>
                <button onClick={() => { setShowAddStudent(false); setStudentSearch(""); }} className="p-1 rounded-md hover:bg-surface-hover transition-colors">
              ...
              <div className="flex-1 overflow-auto">
                {availableStudents.length === 0 ? (
                  <p className="text-center text-sm text-text-muted py-8">Nenhum aluno disponivel. Todos ja estao matriculados ou nao ha alunos cadastrados.</p>
                ) : (
                  <div className="space-y-1">
                    {availableStudents.slice(0, 50).map((s) => (
                      <button key={s.id} onClick={() => handleAddStudent(s.id)} disabled={addingSaving}
                        className="w-full flex items-center justify-between px-4 py-3 rounded-lg hover:bg-surface-hover text-left transition-colors group">
                        <div>
                          <p className="text-sm font-medium text-text-primary">{s.name}</p>
                          <p className="text-xs text-text-muted">{s.email || s.phone || s.cpf || "Sem contato"}</p>
                        </div>
                        <UserPlus className="w-4 h-4 text-text-muted group-hover:text-primary transition-colors" />
                      </button>
                    ))}
                  </div>
                )}
