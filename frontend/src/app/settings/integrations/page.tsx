      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 border-b border-border bg-background-secondary flex items-center px-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold text-text-primary">Integracoes</h1>
          </div>
        </header>
        ...
        <div className="p-6 flex-1 overflow-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.map((item) => (
              <div key={item.name} className="bg-surface rounded-lg border border-border p-5 hover:border-primary/50 transition-colors">
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-border">
                  <span className={`text-xs font-medium ${item.status === "connected" ? "text-green-400" : "text-text-muted"}`}>
                    {item.status === "connected" ? "Conectado" : "Desconectado"}
                  </span>
                  <button className="flex items-center gap-1.5 px-3 py-1.5 bg-surface-hover hover:bg-surface-hover text-text-primary rounded-lg text-xs">
