...
class WhatsAppFlow(models.Model):
    ...
    workflow = models.ForeignKey(Workflow, on_delete=models.CASCADE, related_name="whatsapp_flows")
...
class WhatsAppInteraction(models.Model):
    ...
    flow = models.ForeignKey(WhatsAppFlow, on_delete=models.CASCADE, related_name="interactions")
    ...
    def __str__(self):
        return f"{self.user_phone} - {self.timestamp.strftime('%Y-%m-%d %H:%M')}"


class WhatsAppConversation(models.Model):
    flow = models.ForeignKey(WhatsAppFlow, on_delete=models.CASCADE, related_name="conversations")
    user_phone = models.CharField(max_length=20)
    user_name = models.CharField(max_length=255, blank=True)
    ...
    class Meta:
        ...
        unique_together = ["flow", "user_phone"]

    def __str__(self):
        return f"{self.user_phone} - {self.flow.name}"


class WhatsAppAnalytics(models.Model):
    flow = models.ForeignKey(WhatsAppFlow, on_delete=models.CASCADE, related_name="analytics")
